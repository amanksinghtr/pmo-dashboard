...
- FIX (v13): clean() returned non-string API values (list, dict, int) unchanged.
  If the API returns service_maturity or okrs as a list or object, the JSON
  stored it as an array/object. In the dashboard transformV8, the JS expression
  (e.service_maturity || '').trim() then threw "trim is not a function" because
  a non-empty array is truthy so the || '' fallback was skipped. Fixed by making
  clean() always return a string — lists/dicts are JSON-serialised, other types
  are cast with str().
...
def clean(value):
    """Always returns a clean string regardless of input type.
    If the API returns a list, dict, number, or None for a field that the
    dashboard expects to be a string (e.g. service_maturity, okrs), this
    prevents downstream .trim() TypeError in the JavaScript layer.
    """
    if value is None:
        return ""
    if not isinstance(value, str):
        # Convert lists/dicts to JSON string so content is not silently lost
        try:
            import json as _json
            value = _json.dumps(value, ensure_ascii=False) if isinstance(value, (list, dict)) else str(value)
        except Exception:
            value = str(value)
